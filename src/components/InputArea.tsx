...
import { extractUserDataFromText } from "../modules/valid-guard/extractUserDataFromText";
import type { UserInputData } from "@/types";

interface InputAreaProps {
	onProcess: (data: UserInputData[]) => void;
	isProcessing: boolean;
}

export const InputArea = ({ onProcess, isProcessing }: InputAreaProps) => {
	...
	const handleTextProcess = () => {
		...
		try {
			const extracted = extractUserDataFromText(text);
			if (extracted.length === 0) {
				alert("Nenhum dado encontrado no texto. Formato esperado: NOME CPF DD/MM/AAAA");
				return;
			}
	...
	return (
		<div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
			...
			<div className="flex flex-col gap-4">
				<label
					htmlFor={fileInputId}
					className={`relative rounded-lg border-2 border-dashed transition-colors p-6 flex flex-col items-center justify-center text-center cursor-pointer
            ${dragActive ? "border-indigo-500 bg-indigo-50" : "border-slate-300 hover:border-indigo-400"}`}
				...
				>
					...
					<p className="text-xs text-slate-400 mt-1">
						Formato: NOME CPF DD/MM/AAAA (um por linha)
					</p>
				</label>
				...
				<div className="relative">
					<textarea
						...
						placeholder="Cole sua lista aqui. Formato: NOME CPF DD/MM/AAAA (um por linha). Exemplo: MARIA SUELY DA SILVA ARAUJO BRITO 06819885253 08/07/1957"
						value={text}
